...
package tw.yalan.podcastdemo.player.common
...
import tw.yalan.podcastdemo.player.common.MusicServiceConnection.MediaBrowserConnectionCallback
...
class MusicServiceConnection(context: Context, serviceComponent: ComponentName) {
    ...
    fun addQueueItem(item: MediaDescriptionCompat) {
        mediaController.addQueueItem(item)
    }
...
//            "onConnected token=${mediaBrowser.sessionToken}".loge()
        }
...
//            "$this onPlaybackStateChanged state=${state?.actions}".loge()
            playbackState.postValue(state ?: EMPTY_PLAYBACK_STATE)
        }

        override fun onMetadataChanged(metadata: MediaMetadataCompat?) {
//            "$this onMetadataChanged mediaId=${metadata?.description?.mediaId}".loge()
            nowPlaying.postValue(metadata ?: NOTHING_PLAYING)
        }

        override fun onQueueChanged(queue: MutableList<MediaSessionCompat.QueueItem>?) {
//            "$this onQueueChanged queue=${queue}".loge()
        }

        override fun onSessionEvent(event: String?, extras: Bundle?) {
            super.onSessionEvent(event, extras)
//            "$this onSessionEvent".loge()
            when (event) {
